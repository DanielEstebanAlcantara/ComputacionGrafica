...
Shader::Shader()
{
	shaderID = 0;//incializar variables en cero
	uniformModel = 0;
	uniformProjection = 0;
...
std::string Shader::ReadFile(const char* fileLocation)
{
	...
	if (!fileStream.is_open())//abrir archivo
	{
...
void Shader::CompileShader(const char* vertexCode, const char* fragmentCode)
{
	...
	uniformProjection = glGetUniformLocation(shaderID, "projection");//se agregan las variables uniform desde aquí
	//Ir agregando nuevas variables Uniform
}
...
void Shader::ClearShader()//destructor
{
	if (!shaderID)
	{
